  ...
  shortcuts: [
    ...
    [
      ...
      ([, c], { theme }) => {
        ...
        if (type === "classic") {
          return `grid ${sizeMap[size]}`;
        }
        if (type === "columns") {
          return `grid ${sizeMap[size]}`;
        }
        if (type === "transverse") {
          return `flex flex-row ${theme.breakpoints[size]}`;
        }
        if (type === "vertical") {
          return `flex flex-col ${theme.breakpoints[size]}`;
        }
        // return `flex ${theme.breakpoints[layout[1]]}`;
      }
    ]
  ],
